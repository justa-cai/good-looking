...
**在线体验：<https://justa-cai.github.io/good-looking/>**

> 摄像头那一栏需要浏览器权限。另外：摄像头模式下画面只在本机处理，一个字节都不出这台设备。
...
功能完整，两条路线端到端跑通，摄像头实时取景可用。

- **在线**：<https://justa-cai.github.io/good-looking/>
- 摄像头模式最初只能用假摄像头（Playwright + ffmpeg 合成的 y4m）实测；
  后来在真 USB 摄像头上跑通了完整链路（10.2 Hz、瞳距 122–133 px、侧脸正确拒判）。
